...
```{r, 1 single variable compared; tempo  sjwsjs}
...
tempogramsfalseforweightless <- singlehumanscience |> 
  tempogram(window_size = 4, hop_size = 2, bpms = 60:240, cyclic = FALSE) |>
...
tempogramstrueforweightless <- singlehumanscience |>
  tempogram(window_size = 4, hop_size = 2, bpms = 60:120, cyclic = TRUE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
  scale_fill_viridis_c(guide = "none") +
...
It's difficult to assess any tempo-related aspects of this song from these 2 tempograms. I know (from the research that I looked at) that the song Weightless is **intendedly** designed to start at 70 BPM and then -after some 5 minutes- slows down towards 60 BPM. Reason: assisting in bringing heartrate down. Yet, hardly -if at all- to be seen in the tempograms: not the 70 BPM start nor the descend to 60. The problem relates to the fact that the song **does** have a (low on loudness but definitely consciously hearable) 4/4 meter-type beat but the chords elements are largely ambient-like synths without clear onsets. There **are** melodical elements but they play a minor role and don't necessarily occur in a structured/repetitive manner on anyone of the 4/4 meter beats.
...
tempogramstrueforjoris <- jorisvoornexample |>
  ...
  labs(x = "Time (s)", title = "Polydub, Joris Voorn", y = "Tempo (BPM)") +
  theme_classic()
  grid.arrange(tempogramsfalseforjoris, tempogramstrueforjoris, ncol = 1)
```
