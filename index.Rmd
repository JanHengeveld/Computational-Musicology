...
In the next tabs to the right, graphs and texts will provide insights into the various aspects of music that I dived into for this project. I hope you find it interesting to read!
...
3.  specific relaxation playlist for humans, based upon scientific research ("Humansscience")
4.  specific relaxation playlist for dogs, based upon (some) scientific research ("Dogsscience")

All 4 are own-made playlists.

The overall corpus is made from songs from many playlists on Spotify that exist for those keywords. It is my working hypothesis that the first two playlists are hardly different and that a high level of anthropomorphism is applicable =\> we assume that what humans define and perceive as relaxing music will be true for dogs also and -hence- playlist 2 is based on similar criteria/elements as for playlist 1.

Multiple research from various disciplines (neurosciences, biology, animal studies) has been done into calming/relaxing effects of music, both for humans as for dogs. For the first group, humans, we know much more given a broader spectrum of feedback: we have language as feedback system whereas with dogs we are limited to observing their behavior and measuring bodily aspects like blood pressure or hormone levels (like cortisol, the 'stress hormone'). 

With music for dogs, research shows that similar aspects towards calming apply as for humans (energy, loudness, pitch, instruments) but also differences (variety, genre, 'nature' sounds). Refer to e.g. [Bowman, 2017] (https://www.sciencedirect.com/science/article/abs/pii/S0031938416306977) wherein is stated that "..the effect of habituation may be reduced by increasing the variety of auditory enrichment provided." (end of Abstract)

Group 3 and 4, defined on certain scientific research, may show significant differences with groups 1 and 2 on a variety of musical 'elements'. Also between groups 3 and 4, I wanted to analyze similarities and -especially- search for differences.
...
### A first  analysis of some single musical elements of the 4 relaxation playlists {data-commentary-width="450"}
...
First, acousticness: a surprising diference between the 2 Dogs playlists and 

why histogram for mode: mode is a discrete value (0 or 1 and nothing inbetween) and that's why a histogram is the better way of displaying



------------------------------------------------------------------------

### Analysis of 4 key variables combined over the 4 playlists {data-commentary-width="450"}
...
What mostly struck me is the wide spread of the songs in the playlist humans, science-based across these 4 variables. As these 10 songs - supposedly- form a top 10 of most relaxing song for humans (specifically: to relief anxiety and stress e.g. pre-surgery) why do they not score similarly or at least near to the nr 1. song on that list: Weightless by Marconi Union? I selected this as the 'archetype' song for this playlist. 

------------------------------------------------------------------------

### Analysis of a few songs in terms of chroma features (providing pitch class insights) as can be seen in chromagrams {data-commentary-width="400"}
...
<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/6kkwzB6hXLIONkEk9JciA6?utm_source=generator" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
...
<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/0SLtSDSjPupBUmrXtUNgPE?utm_source=generator" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
...
For the second cepstrogram: first indications make me think that flatness is indeed high in Totally Beached (an archetype song from the dogs science based playlist). Also a relative high level of c04 which could relate to the relative sharp 'attack' style of the strumming guitar with reggae. (more analysis to follow)
...
clusteringdogsscience_juice <-
  recipe(
    track.name ~
      ...
      c01 + c02 + c03 + c04 + c05 + c06 +
      c07 + c08 + c09 + c10 + c11 + c12,
    data = clusteringdogsscience
  ) |>
  step_center(all_predictors()) |>
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(clusteringdogsscience |> mutate(track.name = str_trunc(track.name, 5000))) |>
  juice() |>
  column_to_rownames("track.name")
```
